...
func TestRabbitMQHandler_CreateCategory(t *testing.T) {
	// Setup Dependencies
	...
	cmd := domain.CategoryCreateEvent{
		Name:        "Async Category",
	...
	err = ch.Publish(
		...
		"cmd.catalog.category.create", // routing key
		false,
		false,
		amqp.Publishing{
	...
	assert.Len(t, list, 1)
	assert.Equal(t, "Async Category", list[0].Name)
}
